...
import type { StateOption, DistrictOption, TalukaOption, CityOption, TrainingCenterFormData } from '../types';
...
export default function TrainingCenterForm({ tc, states, districts, talukas, cities }: Props) {
    const isEdit = Boolean(tc?.id);

    const { data, setData, post, put, processing, errors } = useForm<TrainingCenterFormData>({
        tc_id: tc?.tc_id ?? '',
        sip_id: tc?.sip_id ?? '',
        name: tc?.name ?? '',
        address: tc?.address ?? '',
        ...
        taluka_id: tc?.taluka_id ?? '',
        city_id: tc?.city_id ?? '',
        pin_code: tc?.pin_code ?? '',
        email: tc?.email ?? '',
        mobile: tc?.mobile ?? '',
        website: tc?.website ?? '',
        spoc_name: tc?.spoc_name ?? '',
        spoc_mobile: tc?.spoc_mobile ?? '',
        authorized_person_name: tc?.authorized_person_name ?? '',
        authorized_mobile: tc?.authorized_mobile ?? '',
        password: '',
        status: tc?.status ?? true,
    });
    ...
    const onSubmit = (e: FormEvent) => {
        e.preventDefault();
        if (isEdit) {
            put(`/tp/training-centers/${tc.id}`);
        } else {
            post('/tp/training-centers');
        }
    };
        ...
        <form onSubmit={onSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="space-y-2">
                    ...
                    <Input id="tc_id" value={data.tc_id} onChange={(e) => setData('tc_id', e.target.value)} required />
                ...
                <div className="space-y-2">
                    <Label htmlFor="authorized_mobile">Authorized Person Mobile</Label>
                    <Input
                        id="authorized_mobile"
                        value={data.authorized_mobile}
                        onChange={(e) => setData('authorized_mobile', e.target.value)}
                    />
                    <InputError message={errors.authorized_mobile} />
                </div>
                <div className="space-y-2">
            ...
            <div className="flex justify-end gap-2 mt-4">
                <Button type="submit" disabled={processing}>
                    {isEdit ? 'Update Training Center' : 'Create Training Center'}
                </Button>
            </div>
        </form>
    );
